...
func (m *Module) setupPolling() {
	...
	if listError != nil {
		m.j.Log.Error("YouTube", "Unable to find any live streams. "+listError.Error())
		return
	}
	m.liveChatID = listResponse.Items[0].Snippet.ChannelId

	// Create Ticker
	youtubePollingFrequency, youtubePollingError := time.ParseDuration(strconv.Itoa(m.settings.PollingFrequency) + "s")
	if youtubePollingError != nil {
		youtubePollingFrequency, _ = time.ParseDuration("5s")
	}
	m.j.Log.Message("YouTube", "Starting polling at "+youtubePollingFrequency.String())
	m.ticker = time.NewTicker(youtubePollingFrequency)
	m.Poll(false)
	go m.loop()
}
